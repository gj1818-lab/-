...
>
> Source: https://code.claude.com/docs/en/remote-control (snapshot: 2026-08-25)

## 한글 요약

Remote Control(원격 제어)은 **내 컴퓨터에서 실행 중인 Claude Code 세션을 휴대폰·태블릿·다른 브라우저에서 이어서 조작**할 수 있게 해 주는 기능입니다. 코드 실행과 파일 접근은 전부 내 컴퓨터에서 그대로 일어나고, 웹(claude.ai/code)과 모바일 앱은 그 로컬 세션을 들여다보는 창 역할만 합니다. (클라우드에서 실행되는 "Claude Code on the web"과의 차이가 바로 이 점입니다.)

**시작 방법 (셋 중 하나):**

| 방법 | 명령어 | 특징 |
| --- | --- | --- |
| 서버 모드 | `claude remote-control` | 터미널이 서버로 대기, 여러 세션 동시 운영 가능. 스페이스바로 QR 코드 표시 |
| 대화형 세션 | `claude --remote-control` (또는 `--rc`) | 터미널에서도 입력하면서 원격에서도 조작 가능 |
| 기존 세션에서 | `/remote-control` (또는 `/rc`) | 현재 대화 내용을 그대로 가지고 원격 연결. VS Code 확장에서도 동일 |

**다른 기기에서 접속:** 표시된 세션 URL을 브라우저로 열거나, QR 코드를 휴대폰으로 스캔하거나, claude.ai/code / Claude 앱의 세션 목록에서 이름으로 찾으면 됩니다.

**주요 포인트:**

* **요구 사항**: Pro/Max/Team/Enterprise 구독 + claude.ai 로그인(`/login`) 필요. API 키는 지원 안 함. Team/Enterprise는 관리자(Owner)가 관리 설정에서 토글을 켜야 사용 가능
* **자동 연결**: `/config`에서 "Enable Remote Control for all sessions"를 켜면 모든 대화형 세션이 자동으로 원격 연결됨
* **서버 중단 후 복구**: `claude remote-control`을 같은 폴더에서 다시 실행하면 세션 복귀 (중단 후 약 4시간 이내). `--continue` 또는 `--session-id <id>`로 특정 세션만 복귀 가능
* **모바일 푸시 알림**: 긴 작업이 끝나거나 승인이 필요할 때 휴대폰으로 알림. `/config`에서 켜기
* **보안**: 아웃바운드 HTTPS만 사용(내 컴퓨터에 포트 안 열림). 연결 중에는 대화 내용이 Anthropic 서버에 저장되어 기기 간 동기화에 사용됨
* **제한**: 로컬 `claude` 프로세스가 살아 있어야 함(터미널 닫으면 오프라인). SSH 원격 머신이라면 `tmux`/`screen` 안에서 실행 권장. `/plugin`, `/resume` 등 일부 명령은 터미널 전용

아래는 공식 문서 전문(영문)입니다.

---

> **Note:** Remote Control is available on all plans. On Team and Enterprise, it is off by default until an Owner enables the Remote Control toggle in [Claude Code admin settings](https://claude.ai/admin-settings/claude-code).
...
<span id="session-ended-elsewhere"></span>Read the reason before you reconnect. When the session was taken over or ended from another device, app, or Claude Code session, or the server can't find it, the reason says which, and Claude Code leaves out its usual advice to run `/remote-control`:
...
<span id="resume-outcomes"></span>When you resume, you can also get one of these outcomes instead of this message:
