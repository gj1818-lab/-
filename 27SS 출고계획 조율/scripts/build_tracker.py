...
headers=["NO","MD","DS","Style Code","품명","컬러 구성","시즌","생산형태 (수/C/완)","출고차순","출고일(주차)","출고예정일",
         ...
         "비수기 선정","작지발행","사양확정 (전산 8/6)","소재입고 예정/실제일","소재입고 책임","생산처","원산지",
         "기획수량","원가합 (원)","소매가합 (원)","납기1차 (전산참고)","리스크 사유","판정","조율상태","메모/조치사항"]
INPUT={'J','P','Q','R','S','AC','AD'}
ws.freeze_panes='F5'
...
for i,r in enumerate(rows):
    ...
    M_f=(f'=IF($L{rw}="","",IF(OR(ISNUMBER(SEARCH("중국",$V{rw})),ISNUMBER(SEARCH("베트남",$V{rw}))),'
         f'MAX(0,MIN($L{rw},기준정보!$C$9)-MAX($L{rw}-기준정보!$C$6,기준정보!$C$8)+1),'
         f'IF(ISNUMBER(SEARCH("한국",$V{rw})),MAX(0,MIN($L{rw},기준정보!$C$11)-MAX($L{rw}-기준정보!$C$6,기준정보!$C$10)+1),0)))')
    vals={
     ...
     'U':r['생산처'],'V':r['원산지'],'W':int(r['기획수량'] or 0),
     'X':int(r['원가합'] or 0),'Y':int(r['소매가합'] or 0),'Z':nap_v,
     'AA':(f'=IF($K{rw}="","출고일 미정",IF($Q{rw}="X","작지 미발행",IF($Q{rw}="부분","작지 일부 미발행",'
          f'IF(AND($S{rw}<>"",$S{rw}>$N{rw}),"소재입고 마감("&TEXT($N{rw},"MM/DD")&") 초과",'
          f'IF($Q{rw}="","발행여부 확인 필요",IF($S{rw}="","소재 입고일정 확인 필요","정상"))))))'),
     'AB':(f'=IF($K{rw}="","",IF(OR(LEFT($AA{rw},2)="작지",LEFT($AA{rw},4)="소재입고"),'
          f'IF(LEFT($G{rw},1)="S","★ 납기위험(S코드)","◆ 지연주의"),IF($AA{rw}="정상","✓ 정상","? 확인필요")))'),
     'AC':None,'AD':None,
    }
    for col,v in vals.items():
        ...
        if col in ('K','L','N','S') or (col=='Z' and isinstance(v,datetime.datetime)): c.number_format='yyyy-mm-dd'
        if col=='O': c.number_format='0;[RED]-0'
        if col=='M': c.number_format='0'
        if col in ('W','X','Y'): c.number_format='#,##0'
        if col in ('A','G','I','J','M','O','P','Q','R','AB','AC'): c.alignment=Alignment(horizontal='center')

last=HR+N
for f1,rng in [('"O,X"',f'P{HR+1}:P{last}'),('"O,부분,X"',f'Q{HR+1}:Q{last}'),('"O,부분,X"',f'R{HR+1}:R{last}'),('"미조율,조율중,조율완료"',f'AC{HR+1}:AC{last}')]:
...
cf('AB',"★","F8CBCC",RED); cf('AB',"◆","FDE9CC","9C5700"); cf('AB',"✓",TOTALG,BRAND); cf('AB',"?","EEEEEE","666666")
cf('P',"O","DCEDF7",NAVY)
widths={'A':5,'B':9,'C':9,'D':13,'E':30,'F':26,'G':9,'H':13,'I':9,'J':12,'K':12,'L':14,'M':9,'N':14,'O':9,'P':9,'Q':9,'R':11,'S':13,'T':13,'U':12,'V':10,'W':9,'X':14,'Y':15,'Z':13,'AA':26,'AB':15,'AC':9,'AD':24}
for col,wd in widths.items(): ws.column_dimensions[col].width=wd
ws.auto_filter.ref=f'A{HR}:AD{last}'

TRK="'소재역산 트래커'"
RNG_D=f'{TRK}!$D$5:$D${last}'; RNG_W=f'{TRK}!$W$5:$W${last}'; RNG_X=f'{TRK}!$X$5:$X${last}'; RNG_Y=f'{TRK}!$Y$5:$Y${last}'; RNG_P=f'{TRK}!$P$5:$P${last}'
...
kpis=[("전체 스타일",f'=COUNTA({TRK}!$D$5:$D${last})',BRAND),
      ...
      ("★ 납기위험(S코드)",f'=COUNTIF({TRK}!$AB$5:$AB${last},"★*")',RED),
      ("◆ 지연주의(M/X)",f'=COUNTIF({TRK}!$AB$5:$AB${last},"◆*")',ORANGE),
...
for ri,md in enumerate(mds,start=11):
    ...
    ws[f'H{ri}']=f'=COUNTIFS({TRK}!$B$5:$B${last},$B{ri},{TRK}!$AB$5:$AB${last},"★*")'
    ws[f'I{ri}']=f'=COUNTIFS({TRK}!$B$5:$B${last},$B{ri},{TRK}!$AC$5:$AC${last},"조율완료")'
    for col in 'BCDEFGHI':
